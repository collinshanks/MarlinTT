...
void safe_delay(millis_t ms) {
    ...
    delay_ms(ms);
    thermalManager.task();  // This keeps us safe if too many small safe_delay()
                            // calls are made
}
...
void Marlin::wait_for_user_response(millis_t ms /*=0*/,
                                    const bool no_sleep /*=false*/) {
    UNUSED(no_sleep);
...
#define pgm_read_pin(P) \
    (sizeof(pin_t) == 2 ? (pin_t)pgm_read_word(P) : (pin_t)pgm_read_byte(P))
    for (uint8_t i = 0; i < COUNT(sensitive_dio); ++i)
        if (pin == pgm_read_pin(&sensitive_dio[i]))
            return true;
    for (uint8_t i = 0; i < COUNT(sensitive_aio); ++i)
        if (pin == analogInputToDigitalPin(pgm_read_pin(&sensitive_aio[i])))
            return true;
    return false;
}
 ...
 */
bool Marlin::printingIsActive() {
    return !did_pause_print && printJobOngoing();
}
...
void Marlin::startOrResumeJob() {
    if (!printingIsPaused()) {
        ...
        TERN_(HAS_PRUSA_MMU3,
              MMU3::operation_statistics.reset_per_print_stats());
    }
...
inline void abortSDPrinting() {
    ...
    TERN(HAS_CUTTER, cutter.kill(),
         thermalManager
             .zero_fan_speeds());  // Full cutter shutdown including ISR control
...
void Marlin::manage_inactivity(const bool no_stepper_sleep /*=false*/) {
    ...
    const bool has_blocks =
        planner.has_blocks_queued();  // Any moves in the planner?
    if (has_blocks)
        gcode.reset_stepper_timeout(
            ms);  // Reset timeout for M18/M84, M85 max 'kill', and laser.
...
// M18 / M84 : Handle steppers inactive time timeout
    ...
    if (gcode.stepper_inactive_time) {
        ...
        if (!has_blocks && !do_reset_timeout &&
...
// Check if the kill button was pressed and wait to ensure the signal is not
// noise typically caused by poor insulation and grounding on LCD cables. Lower
// numbers here will increase response time and therefore safety rating. It is
// recommended to set this as low as possible without false triggers.
// -------------------------------------------------------------------------------
#ifndef KILL_DELAY
#define KILL_DELAY 250
    ...
    static int killCount =
        0;  // make the inactivity button a bit less responsive
    if (kill_state())
        killCount++;
    else if (killCount > 0)
        killCount--;
...
// Handle the FREEZE button
    ...
    if (!card.isStillPrinting() &&
        !READ(HOME_PIN)) {  // HOME_PIN goes LOW when pressed
...
#define HAS_CUSTOM_USER_BUTTON(N)                               \
    (PIN_EXISTS(BUTTON##N) && defined(BUTTON##N##_HIT_STATE) && \
     defined(BUTTON##N##_GCODE))
#define HAS_BETTER_USER_BUTTON(N) \
    HAS_CUSTOM_USER_BUTTON(N) && defined(BUTTON##N##_DESC)
...
#define CHECK_BETTER_USER_BUTTON(N) \
    _CHECK_CUSTOM_USER_BUTTON(      \
        N, if (strlen(BUTTON##N##_DESC)) LCD_MESSAGE_F(BUTTON##N##_DESC))
    ...
    TERN_(USE_CONTROLLER_FAN,
          controllerFan.update());  // Check if fan should be turned on to cool
                                    // stepper drivers down

    TERN_(AUTO_POWER_CONTROL,
          powerManager.check(!ui.on_status_screen() || printJobOngoing() ||
                             printingIsPaused()));
    ...
    if (ELAPSED(ms, powerManager.last_state_change_ms, PS_EDM_RESPONSE) &&
        (READ(PS_ON_PIN) != READ(PS_ON_EDM_PIN) ||
         TERN0(PSU_OFF_REDUNDANT,
               extDigitalRead(PS_ON1_PIN) != extDigitalRead(PS_ON1_EDM_PIN))))
        kill(GET_TEXT_F(MSG_POWER_EDM_FAULT));
#endif

#if ENABLED(EXTRUDER_RUNOUT_PREVENT)
    if (thermalManager.degHotend(active_extruder) > (EXTRUDER_RUNOUT_MINTEMP) &&
        ELAPSED(ms, gcode.previous_move_ms,
                SEC_TO_MS(EXTRUDER_RUNOUT_SECONDS)) &&
        !planner.has_blocks_queued()) {
        const int8_t e_stepper =
            TERN(HAS_SWITCHING_EXTRUDER, active_extruder >> 1, active_extruder);
        const bool e_off = !stepper.AXIS_IS_ENABLED(E_AXIS, e_stepper);
        if (e_off)
            stepper.ENABLE_EXTRUDER(e_stepper);
        ...
        if (e_off)
            stepper.DISABLE_EXTRUDER(e_stepper);
    ...
    if (ELAPSED(ms, FET_next)) {
        FET_next =
            ms + FET_SAFETY_DELAY;  // 2µs pulse every FET_SAFETY_DELAY mS
...
void Marlin::idle(const bool no_stepper_sleep /*=false*/) {
    ...
    if (++idle_depth > 5)
        SERIAL_ECHOLNPGM("Marlin::idle() call depth: ", idle_depth);
#endif
...
// Handle filament runout sensors
    ...
    if (TERN1(HAS_PRUSA_MMU2, !mmu2.enabled()) &&
        TERN1(HAS_PRUSA_MMU3, !mmu3.enabled()))
        runout.run();
#endif
 ...
 */
void Marlin::kill(FSTR_P const lcd_error /*=nullptr*/,
                  FSTR_P const lcd_component /*=nullptr*/,
                  const bool steppers_off /*=false*/) {
    ...
    if (lcd_error) {
        SERIAL_ECHO_START();
        SERIAL_ECHOLN(lcd_error);
    }

#if HAS_DISPLAY
    ui.kill_screen(lcd_error ?: GET_TEXT_F(MSG_KILLED),
                   lcd_component ?: FPSTR(NUL_STR));
#else
    UNUSED(lcd_error);
...
void Marlin::minkill(const bool steppers_off /*=false*/) {
    ...
    steppers_off ? stepper.disable_all_steppers()
    ...
    while (TERN0(HAS_KILL, kill_state()) ||
           TERN0(SOFT_RESET_ON_KILL, ui.button_pressed()))
        hal.watchdog_refresh();

    // Wait for either KILL or ENC to be pressed again
    while (TERN1(HAS_KILL, !kill_state()) &&
           TERN1(SOFT_RESET_ON_KILL, !ui.button_pressed()))
        hal.watchdog_refresh();
...
void Marlin::stop() {
    ...
    if (!isStopped()) {
        ...
        safe_delay(
            350);  // Allow enough time for messages to get out before stopping
        setState(MF_STOPPED);
    }
}  // Marlin::stop()
...
void Marlin::setup() {
    ...
    tmc_standby_setup();  // TMC Low Power Standby pins must be set early or
                          // they're not usable
...
#define BAUDRATE_2 BAUDRATE
    ...
    while (!MYSERIAL2.connected() &&
           PENDING(get_tick_ms(), serial_connect_timeout)) { /*nada*/
    }
#ifdef SERIAL_PORT_3
#ifndef BAUDRATE_3
...
// Disable any hardware debug to free up pins for IO
    ...
    TERN_(DYNAMIC_VECTORTABLE,
          hook_cpu_exceptions());  // If supported, install Marlin exception
                                   // handlers at runtime
...
// Init and disable SPI thermocouples; this is still needed
    ...
    (TEMP_SENSOR_IS_MAX_TC(REDUNDANT) && REDUNDANT_TEMP_MATCH(SOURCE, E0))
    OUT_WRITE(TEMP_0_CS_PIN, HIGH);  // Disable
#endif
#if TEMP_SENSOR_IS_MAX_TC(1) || \
    (TEMP_SENSOR_IS_MAX_TC(REDUNDANT) && REDUNDANT_TEMP_MATCH(SOURCE, E1))
    OUT_WRITE(TEMP_1_CS_PIN, HIGH);
#endif
#if TEMP_SENSOR_IS_MAX_TC(2) || \
    (TEMP_SENSOR_IS_MAX_TC(REDUNDANT) && REDUNDANT_TEMP_MATCH(SOURCE, E2))
    ...
    OUT_WRITE(SMART_EFFECTOR_MOD_PIN,
              LOW);  // Put Smart Effector into NORMAL mode
#endif
    ...
    SERIAL_ECHO_MSG(" Last Updated: " STRING_DISTRIBUTION_DATE
                    " | Author: " STRING_CONFIG_H_AUTHOR);
#endif
    SERIAL_ECHO_MSG(" Compiled: " __DATE__);
    SERIAL_ECHO_MSG(STR_FREE_MEMORY, hal.freeMemory(), STR_PLANNER_BUFFER_BYTES,
...
#if ENABLED(USE_CONTROLLER_FAN)  // Set up fan controller to initialize also the
...
// Prepare some LCDs to display early
    ...
    SETUP_RUN(settings.first_load());  // Load data from EEPROM if available (or
                                       // use defaults) This also updates
                                       // variables in the planner, elsewhere

#if ENABLED(CONFIGURABLE_MACHINE_NAME)
    SETUP_RUN(
        ui.reset_status(false));  // machine_name Initialized by settings.load()
#endif
    ...
    SETUP_RUN(refresh_delta_clip_start_height());  // Init safe delta height
                                                   // without soft endstops
#endif
    ...
    OUT_WRITE(COOLANT_FLOOD_PIN,
              COOLANT_FLOOD_INVERT);  // Init Flood Coolant OFF
#endif
...
// Initialize nozzle servo(s)
    ...
    SETUP_RUN(hal.watchdog_init());  // Reinit watchdog after
                                     // hal.get_reset_source call
#endif
...
#if HAS_PRUSA_MMU3
    if (mmu3.mmu_hw_enabled)
        SETUP_RUN(mmu3.start());
    ...
    SETUP_RUN(password.lock_machine());  // Will not proceed until correct
                                         // password provided
#endif

#if ALL(HAS_MARLINUI_MENU, TOUCH_SCREEN_CALIBRATION) && \
    ANY(TFT_CLASSIC_UI, TFT_COLOR_UI)
    ...
    for (uint8_t i = 0; i < COUNT(tune) - 1; i += 2)
        BUZZ(tune[i + 1], tune[i]);
#endif
