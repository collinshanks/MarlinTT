...
//
// The range of the timer can be queried at runtime using
// IS_TIM_32B_COUNTER_INSTANCE. This is a more expensive check than a simple
// compile-time constant, so its implementation is deferred until the desire for
// a 32-bit range outweighs the cost of adding a run-time check and
// HAL_TIMER_TYPE_MAX is refactored to allow unique values for each timer.
