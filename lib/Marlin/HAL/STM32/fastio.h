...
#define WRITE(IO,V)             gpio_write(IO, V)
#define READ(IO)                gpio_read(IO)
#define TOGGLE(IO)              gpio_toggle(IO)

#define OUT_WRITE(IO,V)         gpio_write(IO, V)
#define OUT_WRITE_OD(IO,V)      do{ gpio_mode(IO, GPIO_OUTPUT_OD); gpio_write(IO, V); }while(0)
...
#define extDigitalWrite(IO,V) gpio_write(IO, V)
