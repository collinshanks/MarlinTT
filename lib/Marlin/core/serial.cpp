...
uint8_t marlin_debug_flags =
    TERN(MARLIN_DEV_MODE, MARLIN_DEBUG_ECHO, MARLIN_DEBUG_NONE);
...
MAP(_N_STR, LOGICAL_AXIS_NAMES);
MAP(_SP_N_STR, LOGICAL_AXIS_NAMES);
MAP(_N_LBL, LOGICAL_AXIS_NAMES);
MAP(_SP_N_LBL, LOGICAL_AXIS_NAMES);
...
SerialOutputT multiSerial(SERIAL_LEAF_1 REPEAT_S(2, INCREMENT(NUM_SERIAL),
                                                 _S_LEAF));
...
void SERIAL_ECHO_P(PGM_P pstr) {
    while (const char c = pgm_read_byte(pstr++))
        SERIAL_CHAR(c);
}
...
void _print_xyz(NUM_AXIS_ARGS_(const float) FSTR_P const prefix) {
    ...
    SERIAL_ECHOPGM_P(NUM_AXIS_PAIRED_LIST(
        SP_X_STR, x, SP_Y_STR, y, SP_Z_STR, z, SP_I_STR, i, SP_J_STR, j,
        SP_K_STR, k, SP_U_STR, u, SP_V_STR, v, SP_W_STR, w));
#endif
}
...
void print_xyze(LOGICAL_AXIS_ARGS_(const float)
                    FSTR_P const prefix /*=nullptr*/,
                FSTR_P const suffix /*=nullptr*/) {
    _print_xyz(NUM_AXIS_LIST_(x, y, z, i, j, k, u, v, w) prefix);
#if HAS_EXTRUDERS
